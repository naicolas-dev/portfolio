...
export function LocationIndicator({ language = 'pt' }: { language?: 'pt' | 'en' }) {
    ...
    return (
        ...
        >
            <div className="relative overflow-hidden h-9 px-4 rounded-full bg-zinc-900 border border-zinc-800 sm:backdrop-blur-sm hover:bg-zinc-800 hover:border-zinc-700 transition-all duration-500 ease-out shadow-sm flex items-center justify-center min-w-[200px]">
                ...
                <div className={`absolute inset-0 flex items-center justify-center gap-2 transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${actuallyShowingTime ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'
                    ...
                    <span className="text-zinc-500 text-xs font-mono font-medium tracking-wider whitespace-nowrap">
                        {time} <span className="text-zinc-500/60 ml-1">GMT-3</span>
                    </span>
                </div>
            </div>
        </div>
